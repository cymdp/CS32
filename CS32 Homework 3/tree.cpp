...
//
  ...
  // Return the number of ways that all n2 elements of a2 appear
	...
	{
        if (n2<=0) //base case, empty sequence but stil exists
        {
            return 1;
        }
        
        if (n1<=1) //if n1 is at most 1, we are at the end of a1
        {
            if (n2==1 && a1[0]==a2[0])
            {
                return 1;
        ...
        if (a1[0]==a2[0]) //if first elements of a1 and a2 are the same, perform both recur functions
        {
        ...
        {
            return countContains(a1+1,n1-1,a2,n2); //move a1 until it matches with first element of a2
        }
	}
	...
	{
        if (n<=1)
	    return;  //Base Case If n <= 1, do nothing.
        
        //used quicksort method
        int firstNotLess=-1;
        int firstGreater=n;
        int midpoint=(n+1)/2;
        
        divide(a,n,a[midpoint],firstNotLess,firstGreater); //divide w/ respect to midpoint
        order(a, firstNotLess); //sort left half recursively
        order(a+firstNotLess,n-firstGreater); //sort right half recursively
	}
